  ...
  return (
    <EmployeeLayout>
      <div className="flex flex-col gap-6 animate-fade-in max-w-lg mx-auto">
        {/* Page Header */}
        <div>
          <h1 className="text-2xl font-black text-slate-900">My Tables</h1>
          <p className="text-xs text-slate-500 mt-0.5">Tables assigned to you for service</p>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-2 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="glass rounded-3xl p-5 border border-slate-200 bg-white flex flex-col gap-2 shadow-sm">
          ...
          <div className="flex flex-col items-center justify-center gap-4 py-16 text-center bg-white border border-slate-200 rounded-3xl shadow-sm p-6">
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center bg-slate-100 border border-slate-200 text-3xl">
              🪑
            </div>
            <div>
              <h3 className="text-base font-bold text-slate-900">No Assigned Tables</h3>
          ...
          <div className="grid grid-cols-2 gap-4">
            {assignedTables.map((table) => (
              <div
                key={table.id}
                className="glass rounded-3xl p-5 border border-slate-200 bg-white shadow-sm flex flex-col gap-2 transition-all hover:scale-[1.01]"
              >
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Table Code: {table.code}</span>
