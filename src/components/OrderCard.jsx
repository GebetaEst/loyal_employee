...
function getAvailableAction(order, employee) {
  if (!order || !employee) return null;

  // Chef has no operational order controls
  if (employee.role === 'chef') return null;
  ...
  if (Array.isArray(steps) && steps.length > 0) {
    ...
    const label = currentStep.actionLabel || (
      currentStep.key === 'placed' ? 'Mark Served' :
      currentStep.key === 'served' ? 'Complete Order' :
      `Mark ${nextStep.label}`
    );
  ...
  // Fallback if workflow steps are not explicitly defined in REST response
  if (employee.role === 'waiter') {
    if (order.service?.waiter) {
      const waiterId = order.service.waiter.id || order.service.waiter._id || order.service.waiter;
      const employeeId = employee.id || employee._id;
      if (waiterId && employeeId && waiterId.toString() !== employeeId.toString()) {
        return null;
    ...
    if (order.currentStepKey === 'placed') {
      return {
        canAdvance: true,
        label: 'Mark Served',
        nextStep: { key: 'served', label: 'Served' }
      };
    }
    if (order.currentStepKey === 'served') {
      return {
        ...
        nextStep: { key: 'completed', label: 'Completed' }
      };
    }
  }
...
export default function OrderCard({ order, onRefresh }) {
  ...
  const handleAdvance = async () => {
    ...
    try {
      ...
      const response = await api.post(`/api/employee/orders/${orderId}/advance`, {
        expectedStep: order.currentStepKey
      });
      if (response.data?.success) {
        toast.success(`Order transitioned to ${availableAction.nextStep?.label || 'next stage'}!`);
  ...
  const getStatusBadge = () => {
    ...
    switch (status) {
      case 'placed':
        return <span className="px-2.5 py-1 text-xs font-bold rounded-lg bg-amber-50 text-amber-800 border border-amber-200">Placed</span>;
      case 'served':
        return <span className="px-2.5 py-1 text-xs font-bold rounded-lg bg-blue-50 text-blue-800 border border-blue-200">Served</span>;
      case 'completed':
        return <span className="px-2.5 py-1 text-xs font-bold rounded-lg bg-slate-100 text-slate-700 border border-slate-200">Completed</span>;
      default:
        return <span className="px-2.5 py-1 text-xs font-bold rounded-lg bg-slate-50 text-slate-700 border border-slate-200 capitalize">{status}</span>;
    }
  };
